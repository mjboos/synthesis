...
@memory.cache
def split_fmri_data_model(subj, model, n_splits=10):
    subj_preprocessed_path = spenc_dir+'synthesis/clean_data/'+model+'/sub{0:03d}/'.format(subj)
...
if __name__=='__main__':
    ...
    for i, split in enumerate(split_fmri_data(subj)):
        dump(split,
             spenc_dir+'synthesis/prepro/fmri_subj_{}_split_{}.pkl'.format(subj, i),
             compress=3)
